...
class SocketIOManager: NSObject {
    ...
    func addListeners() {
        ...
        socket.on("s_size_change") { data, ack in
            ...
            if self.bellCircle.setupComplete["gotSize"] == false {
        ...
        socket.on("s_global_state") { data, ack in
            if self.bellCircle.setupComplete["gotBellStates"] == false {
        ...
        socket.on("s_user_entered") { data, ack in
            if self.bellCircle.setupComplete["gotUserEntered"] == false {
        ...
        socket.on("s_audio_change") { data, ack in
            if self.bellCircle.setupComplete["gotAudioType"] == false {
        ...
        socket.on("s_host_mode") { data, ack in
            if self.bellCircle.setupComplete["gotHostMode"] == false {
