...
extension RingingRoomViewModel: SocketIODelegate {
    func userDidEnter(_ ringer: Ringer) {
        if ringer.name == user.username {
            self.ringer = ringer
            send(event: "c_request_global_state", with: ["tower_id": towerInfo.towerID])
        }
        
        guard !users.contains(ringer) else { return }
        users.append(ringer)
        print(users)
        sortUsers()
    }
    ...
    func userDidLeave(_ ringer: Ringer) {
        guard let index = users.firstIndex(where: { $0 == ringer }) else { return }
    ...
    func didAssign(ringerID: Int, to bell: Int) {
        if let ringer = users.first(where: { $0.ringerID == ringerID }) {
