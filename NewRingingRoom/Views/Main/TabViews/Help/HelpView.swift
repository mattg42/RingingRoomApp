...
struct HelpView: View {
    ...
    var body: some View {
        NavigationView {
            MasterHelpView(isPresented: $isPresented)
        }
...
struct MasterHelpView: View {
    ...
    var body: some View {
        Form {
            Section {
                NavigationLink("Quick Start Guide", destination: QuickStartGuideView(isPresented: $isPresented))
                NavigationLink("Advanced Features", destination: AdvancedFeaturesView(isPresented: $isPresented))
                NavigationLink("FAQs",  destination: FAQsView(isPresented: $isPresented))
            }
            Section {
                NavigationLink("Full help document", destination: HelpTextView(isPresented: $isPresented))
            }
        }
        .navigationBarItems(trailing: Button {
        ...
        Form {
            Section {
                ForEach(QuickStartGuideHelpSection.allCases) { helpSection in
                    NavigationLink(helpSection.title, destination: HelpSectionView(helpSection: helpSection, isPresented: self.$isPresented))
                }
            }
            Section {
                NavigationLink("Full Quick Start Guide", destination: ScrollView {
                    QuickStartGuideTextView(isPresented: self.$isPresented)
                }
        ...
        Form {
            ...
            Section {
                NavigationLink("Full Advanced Features", destination: ScrollView {
                    AdvancedFeaturesTextView(isPresented: self.$isPresented)
                }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0.0)    {
                ...
                QuickStartGuideTextView(isPresented: $isPresented)
                
                Text("\nAdvanced Features")
                    .font(.title)
                    .bold()
                
                AdvancedFeaturesTextView(isPresented: $isPresented)
            }
            .padding(.vertical)
        }
        .navigationBarItems(trailing: Button {
